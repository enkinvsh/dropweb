...
## <img src="docs/icons/puzzle.svg" width="24" alt="" /> Музыка (ветка feat/meowzic)

Ветка `feat/meowzic` добавляет в dropweb функции музыкального клиента на карточке `meowzic`. Это экспериментальная сборка для ручной установки: она не входит в основной релиз и не публикуется в Google Play.

Интерфейс, метаданные и поиск работают через Spotify. Пользователь входит со своим аккаунтом и получает доступ к личной медиатеке: сохранённым трекам, плейлистам, альбомам, исполнителям и каталогу. Аккаунт остаётся полноценным: отметка «Мне нравится» сохраняет трек в реальную библиотеку Spotify, добавление чужих плейлистов синхронизируется с профилем, а функция «Радио по треку» запускает персональный микс Spotify.

Аудиопоток извлекается через собственный мост на выходных узлах из каталога YouTube Music. Сопоставление треков выполняется по коду ISRC, точному идентификатору конкретной фонограммы от Spotify. По замерам проекта, обычный текстовый поиск возвращал шесть неверных результатов из десяти (концертные версии, видео с текстом, перевод, clean-версию), тогда как поиск по ISRC находит ровно одну исходную запись. Воспроизведение доступно только при активном VPN-подключении: мост доступен исключительно внутри туннеля.
