...
function Invoke-ImportConnect {
  ...
  try {
    ...
    Add-E2ECheck -Name 'proxy-socks-listening' -Passed ($checkpoint.socksListening -eq $true -and $socksPort -gt 0) -Detail "port=$socksPort listening=$($checkpoint.socksListening)"
    $proxyIp = (& curl.exe -sS --max-time 20 --proxy "socks5h://127.0.0.1:$socksPort" 'https://api.ipify.org').Trim()
    ...
    $leftoverApps = @(Get-Process dropweb, DropwebCore -ErrorAction SilentlyContinue)
    Add-E2ECheck -Name 'post-exit-no-app-or-core-processes' -Passed ($leftoverApps.Count -eq 0) -Detail "count=$($leftoverApps.Count)"
    ...
    if ($appProcess -and -not $appProcess.HasExited) { Stop-ObkatkaAppProcesses }
  }
...
function Invoke-BrokenCore {
  ...
  try {
    ...
    $leftovers = @(Get-Process dropweb, DropwebCore -ErrorAction SilentlyContinue)
    Add-E2ECheck -Name 'broken-core-app-exited-or-killable' -Passed ($leftovers.Count -eq 0) -Detail "leftovers=$($leftovers.Count)"
    $script:Phases.boot = 'PASS'
    $script:Phases.import = 'PASS'
  } catch {
    Write-Host "::error::$($_.Exception.Message)"
...
function Invoke-UpgradePreviousStable {
  ...
  try {
    ...
    $oldPids = @(Get-Process dropweb, DropwebCore -ErrorAction SilentlyContinue | ForEach-Object { $_.Id })
    ...
    $oldProcesses = @(Get-Process -Id $oldPids -ErrorAction SilentlyContinue)
    Add-E2ECheck -Name 'upgrade-no-leftover-old-processes' -Passed ($oldProcesses.Count -eq 0) -Detail "oldPidCount=$($oldPids.Count) survivors=$($oldProcesses.Count)"
